...
void MultiplyMatrix(int[,] matrixOne, int[,] matrixTwo) // функция перемножения матриц
{
    for(int i = 0; i < matrixOne.GetLength(0); i++) // перебор строк
    {
        for(int j = 0; j < matrixOne.GetLength(1); j++) // перебор столбцов
        {
            matrixOne[i, j] = matrixOne[i, j] * matrixTwo[i, j]; // перемножаем значения по индексам в матрицах
        }
    }
}
...
Console.WriteLine(); // отступ
MultiplyMatrix(myMatrixOne, myMatrixTwo); // перемножаем значения
PrintMatrix(myMatrixOne); // вывод